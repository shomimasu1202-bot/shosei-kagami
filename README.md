...
- **通変星**（[`ten_gods.py`](backend/app/engine/ten_gods.py)）: 日主（日干）から見た天干の関係を
  10種（比肩・劫財・食神・傷官・偏財・正財・偏官・正官・偏印・印綬）に分類。
  五行の相生相剋 × 陰陽の同異で決まる。`get_ten_god(day_stem, target_stem)`。
- **今年の運勢**: 流年（`reference_date` の年柱＝既定は今日）の年干と日主の通変星から、
  その年のテーマ文を生成し、鑑定文の末尾「今年の運勢」セクションに追加。
  構造化データ `year_fortune`（`year_ganzhi` / `ten_god` 等）も返す。
  例: 日主 庚・2026年（丙午）→「偏官」→ 挑戦とプレッシャーが力に変わる年。
